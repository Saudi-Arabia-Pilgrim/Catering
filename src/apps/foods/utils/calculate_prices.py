...
class CalculatePrices():
    ...
    def get_menus(cls, foods):
        ...
        menus = list(Menu.objects.filter(foods__in=foods).prefetch_related("foods").distinct())
        return menus
    ...
    def calculate_foods(cls, foods):
        ...
        for food in foods:
            food.change_object()
        Food.objects.bulk_update(foods, ["status", "net_price", "profit", "gross_price"])
    ...
    def calculate_menus(cls, menus):
        ...
        for menu in menus:
                menu.change_object()
        Menu.objects.bulk_update(menus, ["status", "net_price", "profit", "gross_price"])
    ...
    def calculate_recipes(cls, recipes):
        ...
        Recipe.objects.bulk_update(recipes, ["status", "net_price", "profit", "gross_price"])