...
class WarehouseSerializer(CustomModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = ["status", "count", "warehouse_id"]
...
class WarehouseExpensesSerializer(CustomSerializer):
    ...
    def validate(self, validated_data):
        ...
        if count > (warehouse.count * difference_measures if difference_measures else 1)  or count == 0:
            raise CustomExceptionError(code=400, detail="The count exceeds the available stock in the warehouse.")
    ...
    def save(self):
        ...
        warehouse.count = warehouse.count - (self.validated_data["count"] / difference_measures if difference_measures else 1)
        warehouse.save()